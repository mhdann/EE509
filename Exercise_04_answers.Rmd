...
```{r}
# Probit, mean prob circa dia ~ 17
xseq = seq(from = min(dia),to=max(dia),length.out = 300)

g <- function(a0, x){
  # mean fecundity
  a0*x^2  
}
theta <- function(b0,b1,x){
  pnorm(x,mean = b0, sd=b1)
}

# Fecundity
plot(dia, ncone)
lines(xseq, g(param[1],xseq),col=2)
lines(xseq, g(param[1],xseq)*theta(param[2],param[3],xseq),col=3)


# Maturation
plot(dia, cones)
lines(xseq, theta(param[2],param[3],xseq), col=2)
...
likfit = function(param,dia,ncone){
  ...
  b1 = param[3]
  cones      = ncone > 0
  g_hat      = g(a0, dia)           # Fecundity parabola
  theta_hat  = theta(b0, b1, dia)  	# Maturation probit
  prob       = log(((1-theta_hat)*(1-cones) + theta_hat*dpois(ncone,g_hat)))
  # combine with data model Poisson
  return(-sum(prob))
}
```
...
lines(xseq, g(param[1],xseq),col=2)
lines(xseq, g(param[1],xseq)*theta(param[2],param[3],xseq),col=3)
lines(xseq, g(a0,xseq),col=2, lty=2)
lines(xseq, g(a0,xseq)*theta(b0,b1,xseq),col=3, lty=2)

# Maturation
plot(dia, cones)
lines(xseq, theta(param[2],param[3],xseq), col=2)
lines(xseq, theta(b0,b1,xseq), col=2, lty=2)
...
|     Value|   All points|  Ambient| Elevated CO2|
|---------:|------------:|--------:|------------:|
|     $a_0$|    `r a0[1]`|`r a0[2]`|    `r a0[3]`|
|     $b_0$|    `r b0[1]`|`r b0[2]`|    `r b0[3]`|
|     $b_1$|    `r b1[1]`|`r b1[2]`|    `r b1[3]`|
|       $n$|     `r n[1]`| `r n[2]`|     `r n[3]`|
|$\ln \text{L}$|    `r lnL[1]`|`r lnL[2]`|    `r lnL[3]`|
...
```{r}
# Fecundity
plot(dia, ncone,log="y")
lines(xseq, g(a0[1],xseq)*theta(b0[1],b1[1],xseq),col=2, lty=1)
lines(xseq, g(a0[1],xseq),col=2, lty=2)
lines(xseq, g(a0[2],xseq)*theta(b0[2],b1[2],xseq),col=3, lty=1)
lines(xseq, g(a0[2],xseq),col=3, lty=2)
lines(xseq, g(a0[3],xseq)*theta(b0[3],b1[3],xseq),col=4, lty=1)
lines(xseq, g(a0[3],xseq),col=4, lty=2)
legend("topleft",legend=paste(c("all","all","amb","amb","elev","elev"),c("g*theta","g"),sep=", "),col=c(2,2,3,3,4,4),lty=c(1,2,1,2,1,2),title="Fecundity")


# Maturation
plot(dia, cones, ylab="Probability of Maturity", xlab="Diameter")
legend("topleft",legend=c("all","amb","elev"),col=2:4,lty=1, title="Maturation Probit")
lines(xseq, theta(b0[1],b1[1],xseq), col=2)
lines(xseq, theta(b0[2],b1[2],xseq), col=3)
lines(xseq, theta(b0[3],b1[3],xseq), col=4)
