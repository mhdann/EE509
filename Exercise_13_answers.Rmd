...
The third degree polynomial looks good enough. The fourth order polynomial doesn't appear to pick up many more details.
...
gaufit <- function(theta){    ## negative log likelihood function
  d = theta[1]
  alpha = theta[2]
  se = theta[3]
  sigma = theta[4]
...
sphfit <- function(theta){    ## negative log likelihood function
  d = theta[1]
  alpha = theta[2]
  se = theta[3]
  sigma = theta[4]
...
sfit <- optim(c(2,0.1,1,0.5,0.01),sphfit)    ## numerical optimization
sfit
...
```{r}
par(lwd=2,cex=1.1)
cg <- correlogram(surf3,300,xlim=c(0,10))
...
```{r}
data.frame(Model=c("Exponential","Gaussian","Spherical"),ln_likelihood=c(-efit$value,-gfit$value, -sfit$value),AIC=c(2*(4+efit$value),2*(4+gfit$value),2*(5+sfit$value)))
```

The Gaussian fit looks best. But only just.
...
The xp axis (horizontal) represents bin-centers of the correlogram. I believe that was in degrees, so 1 degree bins of the pair-wise distances (lots of pairs in this set).

The vertical represents the covariance between ozone levels at points in the corresponding bin.

The range, distance to assymptote, is about 2 to 2.25 degrees, which is about 200 miles depending on assumptions about whether these are degrees latitude or longitude.
...
```{r}
KrExp <- surf.gls(3,gaucov,dat$Lon,dat$Lat,dat$Mean,
...
(map, see above) Mohave desert, Phoenix area, Salt Lake City, Appalachia, Colorado Springs corridor.
...
```{r}
image(se3,main="Krige",cex.main=2,asp=70/50)                ## image of predictions
contour(se3,add=TRUE,lwd=1) ## conv to ppb                  ## contours of prediction
map("state",add=TRUE)
```

The instructions said to use the Kriging map of mean Ozone along with the contours of the uncertainty. This didn't make much sense so I used the map of the uncertainty and the contours of the uncertainty.

The largest uncertainty lies in the rural areas where the data is sparse: various places throughout the west mostly. Montana/Idaho/Nebraska/Texas Panhandle etc.
...
As per the previous hint, we have not fit the full spatial model to the data map. We've fit a curve to the correlogram which itself is a reduced form of the data. We could fit the surface to the full map.
...
f = 1 results in a line while f >=10 will be overfit.
### Lab Report Task 4
...
The mean of the detrended series is indistinguishable from zero. Variance appears homoskedastic, though for thoroughness a Brusch-Pagan or a White test for homoscedasticity is performed. Notice that there is no detectable heteroscedasticity in time.

```{r}
trend <- 1:length(rx) # set up time variable
fit_rx <- lm(rx~1+trend)
summary(fit_rx)
library(lmtest)
bptest(fit_rx)
```
...
Yes. This first-differenced series is stationary too.
```{r}
trend <- 1:length(dxt) # set up time variable
fit_dxt <- lm(dxt~1+trend)
summary(fit_dxt)
bptest(fit_dxt)
```
...
The ACF of xt demonstrates autocorrelation while the first difference and the trend to not. This indicates that difference or detrending is a useful start (d=1 on the xt series).

The PACF of dxt shows a significant value at lag two, suggesting that at most p = 2.
...
```{r}
# Compare this briefly to d = 0 or p = 1 
df <- data.frame()
for(p in 0:2){
  for(d in 0:1){
    for(q in 0:1){
      arima_res <- arima(xt, c(p,d,q))
      df <- rbind(df, c(p,d,q,arima_res$loglik, arima_res$aic))
    }
  }
}
names(df) <- c("p","d","q","loglik","aic")
df[order(df$aic),]
```

Well, from a naive application of the AIC it seems a simple p=1 suffices. But then I know that the first difference is required for stationarity. I think p=1 yields a comparable because the time frame is so short the trend is essentially linear. Again, though, there isn't much distinction between the first 5 models, and that makes me think that the series is too short to be conclusive. Many of the models appear indistinguishable via the AIC.

With the requirement of differencing for stationarity, the model is probably (2,1,0), though (0,1,1) is not much different.  Purely from the flow chart, I would choose (2,1,0) or (0,1,0). If forced to choose between those, I would choose (2,1,0).
